...
Proteção adicional publicada no commit `f0ee723`: retenção conservadora de 90 dias com pelo menos 7 pares completos, executada somente após novo backup externo confirmado; monitor separado às 11h17 de São Paulo falha quando o par mais recente no Drive ultrapassa 36 horas. Testes isolados, YAML, formatação e consulta somente leitura ao Drive passaram. Execução manual do monitor `35288577700` terminou com sucesso. Execução manual do backup com retenção `35288612776` terminou com sucesso; log confirmou backup externo, `backup:check` e `Retenção concluída: 0 pares antigos removidos`. Listagem posterior mostrou dois pares cifrados no Drive. Nenhum dado do Neon foi escrito e nenhuma cópia foi removida. Primeira execução pelo agendamento ainda pendente. As notificações por e-mail dependem da configuração da conta GitHub; não há monitor externo independente do agendador.
...
Em 17/09, o remoto `drive_salao` foi autorizado, protegido com modo 0600 e validado com envio, leitura idêntica e remoção de arquivo fictício. Usuário confirmou que guardou a chave fora do PC. Três Secrets GitHub necessários foram cadastrados e seus nomes conferidos, sem valores expostos. Workflow publicado em `main`; primeira execução manual falhou porque o validador de URL recusava `channel_binding=require` do Neon, correção publicada no commit `8d024de` e aprovada em `test:backup` completo fora do sandbox. Segunda execução falhou por pg_dump 17.11 contra Neon PostgreSQL 18.6; cliente 18.6 local leu o esquema Neon sem erro. Workflow ajustado localmente para usar 18. O backup real continua **inativo**: ajuste do cliente ainda a publicar, nenhum dump real ou restauração real isolada, variável `BACKUP_ENABLED` ausente. Próximo passo imediato: publicar ajuste, repetir execução manual, conferir cópia e restauração isolada; ativar rotina diária só após ensaio aprovado.

Retomada posterior em 17/09: usuário ativou a Google Drive API em novo projeto Google Cloud e iniciou OAuth Externo/Testando. Criou repositório público `SantosMarcos95/santosmarcos95.github.io` para páginas exigidas pelo Branding. Com autorização do usuário, páginas de apresentação, privacidade e termos foram publicadas no commit `adf81e7`; GitHub Pages confirmou build e HTTPS, e as três URLs retornaram HTTP 200 com texto esperado. Rascunhos locais em `.local/oauth-pages/`. Usuário relatou ter gravado os três links e o domínio autorizado no Branding; isso não foi verificado diretamente no Google Cloud. Ainda não há domínio verificado, Client ID, token rclone ou backup ativo. Próximo passo: publicar app OAuth, criar cliente Desktop e autorizar remoto. Nenhum segredo foi compartilhado.

Atualização da mesma retomada: usuário relatou ter publicado o app OAuth e criado cliente Desktop. O remoto local `drive_salao` já existe, tipo Drive, escopo `drive.file`, modo do arquivo 0600, mas não contém token. Autorização falhou primeiro com `State did not match` após instrução com URL incompleta (corrigida); tentativas seguintes falharam quase imediatamente com `No code returned by remote server`. Rclone local antigo `1.60.1-DEV` foi atualizado para binário oficial `1.75.1` com SHA-256 conferido; versão antiga preservada em `.local/bin/rclone-1.60.1`. Próximo passo: repetir `config reconnect` na versão nova e observar tela do Google/resultado. Nenhuma credencial foi compartilhada, nenhum backup executado.

Nova tentativa Linux 1.75.1 falhou em cerca de dois segundos com `No code returned by remote server`, antes de login; `xdg-open` ausente. Rclone oficial Windows 1.75.1 preparado em `.local/bin/rclone.exe`, checksum conferido; execução Windows e leitura do mesmo remoto via caminho UNC confirmadas. Próximo passo: autorizar com o executável Windows usando `--config "$(wslpath -w "$PWD/.local/rclone.conf")"`, para manter navegador e callback no Windows. Ainda sem token ou backup real.

Autorização com rclone Windows chegou ao Google, que retornou `401 invalid_client` / cliente não encontrado. Inspeção local só de formato encontrou Client ID presente, mas sem sufixo obrigatório `.apps.googleusercontent.com`; Client Secret presente, token ausente. Usuário deve copiar novamente o ID e Secret do cliente Desktop em Google Auth Platform > Clientes para `.local/rclone.conf` (0600), sem compartilhar valores, e repetir o comando Windows. Nenhum backup executado.

Usuário corrigiu Client ID e concluiu autorização no rclone Windows (`Success!`). Token presente em `.local/rclone.conf`; gravação via Windows alterou modo para 0644, corrigido para 0600. Teste Drive apenas com arquivo fictício passou: upload e download de volta via rclone Linux, comparação `cmp` exit 0, remoção do arquivo de teste no Drive. Primeiro upload no sandbox travou por restrição de rede e foi interrompido; repetição escalada passou. A chave de criptografia local de 64 caracteres existe em `.local/backup.env` (0600), mas usuário ainda precisa confirmar cópia fora do computador antes de ativar backup real. Nenhum backup real, Secret GitHub ou agendamento ativo até aqui.

Usuário confirmou cópia da chave fora do PC. Três GitHub Actions Secrets cadastrados por `gh` a partir de arquivos locais e nomes conferidos; nenhum valor exibido. Workflow ajustado para execução manual com agendamento desativado e rclone oficial 1.75.1 com checksum fixo. YAML e `git diff --check` passaram; arquivos privados ignorados pelo Git. Ainda falta publicar o workflow, ensaiar backup real e restauração isolada; variável de agendamento continua ausente.

Ponto de parada anterior: em 17/09, após considerar execução pendente no PC, o usuário escolheu **execução diária na nuvem** e pediu parar na etapa de conectar o Google Drive para continuar mais tarde. Não ativar os modelos locais do Windows/systemd. Workflow GitHub Actions preparado em `.github/workflows/salao-backup.yml`, agendado às 03h17 de São Paulo, com gate `BACKUP_ENABLED=true` ainda ausente. Não houve push, configuração de Secrets, execução real, backup do Neon nem restauração real. Instruções em `docs/backup-cloud-drive.md`. O usuário não deve enviar Client Secret, token ou chave por conversa.

Validações realmente concluídas: `npm run test:backup-job` passou com fixtures; `npm run test:integration` passou em PostgreSQL isolado após repetir fora do sandbox (primeira tentativa falhou somente por socket TCP bloqueado); build incluído nessa suíte passou. YAML analisado por PyYAML, `systemd-analyze verify` e sintaxe PowerShell passaram; Prettier e `git diff --check` passaram. Não há testes ativos. Usuário cancelou os seguimentos sobre comanda e status da agenda; nenhuma alteração funcional desses módulos foi mantida nesta etapa. O backup real permanece **inativo**. A rotina no GitHub pode atrasar ou ser descartada pela plataforma; monitor de ausência de execução e retenção no Drive continuam pendentes.

Ao encerrar em 17/09, o usuário pediu fechar tudo que estivesse rodando. Verificação de processos e portas dentro e fora do sandbox não encontrou processos ou listeners do projeto; somente DNS do ambiente estava ativo. Nenhum processo do projeto precisou ser encerrado.

## Backup no próximo logon — preparação em 17/09/2026

Usuário escolheu executar a pendência quando o computador ligar, confirmou Google Drive e informou que o PC desliga com frequência. Ambiente é WSL Ubuntu sobre Windows; user systemd funciona, mas `Linger=no`. Preparada tarefa Windows diária às 03h e no logon com `run-if-due`, que pula execução se a última cópia externa íntegra tiver menos de 20h. Script local lê `.local/neon.env` existente e validado para Neon; nenhuma migration. `rclone` 1.60.1 instalado em `.local/bin` a partir do pacote Ubuntu. `.local/backup.env` criado modo 0600 com chave 256 bits gerada localmente, sem revelar valor; ainda precisa ser guardada fora do PC. OAuth `.local/rclone.conf` não configurado, tarefa não instalada, backup real não executado. Guia em `docs/backup-wsl-drive.md`. Antes de ativar: criar Client ID próprio do Google Drive, autorizar remoto `drive_salao`, testar envio/recuperação isolada, preservar chave e configurar aviso de falha. O computador desligado não produz novas cópias até próximo logon.

Usuário confirmou venda por unidade/embalagem inteira e que cancelamento de comanda antes de pagamento deve manter o estoque disponível. O código já baixa apenas no primeiro pagamento; teste de regressão adicionado. A primeira execução da suíte de integração esbarrou no bloqueio de socket do sandbox; repetição com permissão escalada em andamento neste marco. Nenhum dado real modificado.
...
# Backup externo — preparação em 16/09/2026

Usuário autorizou implementar backup automático criptografado. Aceitou Google Drive e informou que o computador de backup desliga às vezes; foi perguntado se prefere execução ao ligar ou diária na nuvem, resposta ainda pendente neste marco. Implementado `scripts/lib/offsite-backup.mjs`: cifra AES-256-GCM, envia dump e manifesto por rclone, relê os dois para confirmar SHA-256, registra sucesso só após confirmação. `backup:check` pode exigir cópia externa e o CLI pode enviar POST HTTPS genérico de falha ao webhook configurado. Descriptografia para restauração incluída. Ensaios com arquivos fictícios passaram em `npm run test:backup-job`; nenhuma conexão Drive, chave definitiva, alerta ou agendamento foi ativado. Nenhum dado real foi lido ou modificado. Próximos passos: decidir executor, configurar OAuth Drive e rclone privado, preservar chave, instalar temporizadores ou agendamento na nuvem, testar backup real/restauração isolada e entrega do alerta. Não declarar backup externo operacional antes disso.

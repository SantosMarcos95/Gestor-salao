...
## Ponto de parada pedido pelo usuário — conectar Google Drive

Em 17/09, após considerar execução pendente no PC, o usuário escolheu **execução diária na nuvem** e pediu parar na etapa de conectar o Google Drive para continuar mais tarde. Não ativar os modelos locais do Windows/systemd. Workflow GitHub Actions preparado em `.github/workflows/salao-backup.yml`, agendado às 03h17 de São Paulo, com gate `BACKUP_ENABLED=true` ainda ausente. Não houve push, configuração de Secrets, conexão OAuth, execução real, backup do Neon nem restauração real. A chave aleatória existe apenas em `.local/backup.env` (0600) e precisa ser guardada fora deste computador antes da ativação. `rclone` local existe em `.local/bin/rclone`; `.local/rclone.conf` ainda não existe. O próximo passo é acompanhar o usuário, em etapas, na criação de Client ID OAuth próprio do Google Drive, publicação do app e autorização do remoto `drive_salao`; instruções em `docs/backup-cloud-drive.md`. Depois: teste de arquivo fictício, configurar Secrets com `gh` sem mostrar valores, publicar workflow, executar manualmente, validar Drive e restauração isolada, ativar variável e alertas. O usuário não deve enviar Client Secret, token ou chave por conversa.

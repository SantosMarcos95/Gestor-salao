...
Proteção adicional preparada nesta continuação: retenção conservadora de 90 dias com pelo menos 7 pares completos, executada somente após novo backup externo confirmado; monitor separado às 11h17 de São Paulo falha quando o par mais recente no Drive ultrapassa 36 horas. Testes isolados e consulta somente leitura ao Drive passaram. Publicação e execução manual do monitor ainda pendentes neste ponto. As notificações por e-mail dependem da configuração da conta GitHub; não há monitor externo independente do agendador.

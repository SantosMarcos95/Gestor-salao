...
Em 17/09, o remoto `drive_salao` foi autorizado, protegido com modo 0600 e validado com envio, leitura idêntica e remoção de arquivo fictício. Usuário confirmou que guardou a chave fora do PC. Três Secrets GitHub necessários foram cadastrados e seus nomes conferidos, sem valores expostos. O backup real continua **inativo**: workflow ainda não publicado, nenhum dump real ou restauração isolada, variável `BACKUP_ENABLED` ausente. Próximo passo imediato: publicar workflow, executar manualmente, conferir cópia e restauração isolada; ativar rotina diária só após ensaio aprovado.

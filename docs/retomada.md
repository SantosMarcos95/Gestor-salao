...
Em 17/09, o remoto `drive_salao` foi autorizado, protegido com modo 0600 e validado com envio, leitura idêntica e remoção de arquivo fictício. Usuário confirmou que guardou a chave fora do PC. Três Secrets GitHub necessários foram cadastrados e seus nomes conferidos, sem valores expostos. Workflow publicado em `main`; primeira execução manual falhou porque o validador de URL recusava `channel_binding=require` do Neon. Correção local passou em `test:backup` completo fora do sandbox. O backup real continua **inativo**: correção ainda a publicar, nenhum dump real ou restauração real isolada, variável `BACKUP_ENABLED` ausente. Próximo passo imediato: publicar correção, repetir execução manual, conferir cópia e restauração isolada; ativar rotina diária só após ensaio aprovado.
